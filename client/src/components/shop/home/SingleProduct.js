...
const SingleProduct = (props) => {
  ...
  if (data.loading) {
    return (
      <div className="flex items-center justify-center col-span-2 py-24 md:col-span-3 lg:col-span-4">
        <svg
    ...
    <Fragment>
      {products && products.length > 0 ? (
        products.map((item, index) => {
          return (
            <Fragment key={index}>
              <div className="relative col-span-1 m-2 overflow-hidden rounded-md">
                <div className='overflow-hidden rounded-md'>
                  <img
                    onClick={(e) => history.push(`/products/${item._id}`)}
                    className="object-cover object-center w-full transition-all duration-500 ease-in-out transform rounded-md cursor-pointer hover:scale-110"
